...
class AbstractUploaderTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $uploader = $this->getUploaderWithState(UploadState::ABORTED);
        $uploader->upload();
    }
    ...
    {
        $uploader = $this->getUploaderWithState(UploadState::INITIATED, [
    ...
    public function testCanGetStateFromUploadId()
    {
        $uploader = $this->getTestUploader(Psr7\stream_for(null), [
            'bucket'    => 'foo',
            'key'       => 'bar',
            'upload_id' => 'baz',
        ]);
        ...
        $this->assertEquals(2, $state->getPartSize());
        $this->assertEquals(2, count($state->getUploadedParts()));
    }
    ...
    public function testAsyncUpload()
    {
        ...
        $promise = $uploader->uploadAsync();
        $this->assertSame($promise, $uploader->uploadAsync());
        $this->assertInstanceOf('Aws\Result', $promise->wait());
    }

    public function testAsyncAbort()
    {
        $uploader = $this->getTestUploader(Psr7\stream_for('abcde'), [
            'bucket'    => 'foo',
            'key'       => 'bar',
        ...
        $promise = $uploader->abortAsync();
        $this->assertSame($promise, $uploader->abortAsync());
        $this->assertInstanceOf('Aws\Result', $promise->wait());
    }
